...
import '../data_source/auth_remote_source_data.dart';

@Injectable(as: AuthRepo)
class AuthRepoImp implements AuthRepo {
  final AuthRemoteSourceData authRemoteSourceData;
  AuthRepoImp(this.authRemoteSourceData);

  @override
  Future<Either<ServerFailure, UserEntity>> login(String email, String password) async{
    try{
      final response = await authRemoteSourceData.login(email, password);
  ...
  @override
  Future<Either<ServerFailure, UserEntity>> signUp(String name, String email, String password) async{
    try{
      final response = await authRemoteSourceData.SignUp(name, email, password);
      return Right(response.toEntity());
  ...
  Future<Either<ServerFailure, UserEntity>> getProfile() async{
    try{
      final response  = await authRemoteSourceData.getProfileData();
      return Right(response.toEntity());
    }on ServerFailure catch(error){
      return Left(error);
    }
  ...
  Future<Either<ServerFailure, String>> logout()async {
    try{
      final response  = await authRemoteSourceData.logout();
  ...
  @override
  Future<Either<ServerFailure, String>> updateProfile(UserModel userModel) async{
    try{
      final response  = await authRemoteSourceData.updateProfile(userModel);
      return Right(response);
    } on ServerFailure catch(e){
      return Left(e);
    }
